...
#   setsid nohup script -qec "bash packages/core/scripts/run-overnight.sh" /dev/null >/dev/null 2>&1 &
set -u

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
BUN="$(command -v bun || true)"
LOG="$REPO/output/run-overnight.log"
...
# a 2x2 over model and thinking level, three apps each, one run per cell. every
# batch runs the same spec, so the only variables across the grid are the two
# being compared. batches run in value order: the two low batches first, since
# low is the established setting and the mediums are the re-test.
# medium lost to low on 2026-08-31, but that comparison predates the
# verdict-write fixes, and the timeout path those fixes closed is what killed
# the medium runs. each medium batch pins the same model and context as its low
# counterpart, so thinking level is the only variable within a pair.
# one run per cell, so this sizes and ranks the grid; it does not measure
# variance. the 5-app spec with a repeat is the follow-up once a cell wins.
batch qwen3.6-mtp-tuned  26624 low    experiment-overnight-3apps.json overnight-3.6-low
batch qwen3.8-iq3s-tuned 65536 low    experiment-overnight-3apps.json overnight-3.8-low
batch qwen3.6-mtp-tuned  26624 medium experiment-overnight-3apps.json overnight-3.6-medium
batch qwen3.8-iq3s-tuned 65536 medium experiment-overnight-3apps.json overnight-3.8-medium

log ""
log "==================== $(date -Is)  all batches done, reclassifying"
"$BUN" --cwd packages/core scripts/reclassifyRuns.ts \
  >"$REPO/output/reclassify-overnight.md" 2>>"$LOG"
log "reclassification written to output/reclassify-overnight.md"
log "DONE $(date -Is)"

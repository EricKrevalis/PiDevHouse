...
4. Include only necessary work. Each story: unique positive id, title, description, acceptanceCriteria, blockedBy, status "todo". Give ids in build order — lower ids run first — and add only real, non-circular prerequisites. Every extra story must have a concrete split reason in its scope; otherwise merge it.

...
3. Trace each criterion to production code, implement it, and add the smallest meaningful unit tests. Cover the relevant failure or boundary case. When a dependency is the smallest complete change, add it with `bun install` in the workspace root. The sandbox only writes inside the workspace.

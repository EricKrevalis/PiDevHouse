...
3. Browser UI: clear leftovers first (pkill -f http.server; pkill -f remote-debugging-port; pkill -f agent-browser), then run the whole check in one bash call: start the local server with its existing command (or python3 -m http.server for a static app), launch headless Chromium, drive it via agent-browser, screenshot, clean up. Load the command reference before any agent-browser call so the workflow is current: \`agent-browser skills get core --full\`; follow its snapshot-and-ref flow.
   Sandbox-safe idioms: the sandbox rejects $(...) command substitution, a bare $VAR used as a command name, and > redirection to anything but /dev/null. Stay inside them:
   - Keep the port in an arithmetic variable set by literal assignment (allowed): P=$((RANDOM % 200 + 9200)). Never write it to a file; never capture it with $(...).
   - Write the full \`agent-browser --cdp $P ...\` inline on every line. Never do AB="agent-browser ..."; then $AB ...; a bare $VAR at the start of a command is rejected.
   - To read a printed value, run the command on its own line and read its stdout directly, or pipe to grep to check pass/fail. Never wrap a command in $(...) to capture its output, and never build strings like "display=[$(agent-browser ... get text '#display')]" or code=$(curl ...).
   - For output you only need to discard, redirect to /dev/null (with 2>&1 if needed). Do not redirect to a scratch log file like > /tmp/foo.log 2>&1.
   Working pattern for a static app:
